class Textbook
  ...
  class PageManager
    ...
    def initialize(book, hash)
      @book = book
      if hash
        @start_sheet = hash['start_sheet']
        @start_page = hash['start_page']
        @last_page = hash['last_page']
      else
        @start_sheet, @start_page = 1, 1
      end
    end
    ...
    def each_page(start: nil, stop: nil)
      ...
      @book.each_sheet do |sheet, sheet_num|
    ...
    def last_page
      ...
      return page_num_for(@book.num_sheets)
    end

...
# function. A Curve object consists of a set of raw scores and a cutoff map that
# associates letter grades with minimum raw scores.
#
...
# There might be value to doing so. Consider looking at skewness and kurtosis of
...
class CurveCalculator
  ...
  element(:min_mean, Numeric)
  element(:max_mean, Numeric)
  element(:target_mean, Numeric)
  element(:target_sd, Numeric)
  element(:min_grade, String)

  def receive_min_grade(s)
    @grades = GPA_MAP.keys[0, GPA_MAP.keys.index(s) + 1]
  end

  element(:actual, { String => Numeric }, optional: true)
  attr_reader :actual
  ...
  # mapping letter grades to [high cutoff, low cutoff]. For the top or bottom
  # grade, the relevant cutoff is nil.
  #
  def gpa_cutoffs
  ...
  def each
    ...
    # Every grade but the last is subject to cutoffs; the last grade always has
    # the lowest score as the cutoff.
    mgrades = @grades[0..-2]

    #
    # To compute all cutoffs, imagine an ordered list of the scores, into which
    # each cutoff-relevant letter grade is inserted to mark the points of
    # cutoff. This list now contains:
    #
    #     r = svals.count + mgrades.count - 1
    #
    # items. (A cutoff cannot go after the lowest score because setting a
    # grade's cutoff to a value below the lowest score is equivalent to setting
    # the cutoff to the lowest score; that's what the -1 is for.) Positioning
    # the letter grades is thus equivalent to n Choose r, where n is the number
    # of grades.
    #
    ...
    (0...(svals.count + mgrades.count - 1)).to_a.combination(
      ...
      # be removed. The third value must be decremented by two, and so on. Thus,
      # the index of mgrades (i) is subtracted from each value below.
  ...
  def measure_gpa_range(curve)
    ...
    if gpa_mean > @max_mean
      ...
      curve.metrics[:mean] = (@target_mean - gpa_mean).abs / (@max_mean - @min_mean)
    end
  end

  #
  # Determines how far off of an ideal distribution this curve is. We compute
  # the squares of differences between the ideal and actual number of each grade
  # to award, and divide by the total grades awarded. This produces a score
  # where zero is a perfect match.
  #
  def measure_distribution(curve)
    curve.metrics[:dist] = Math.sqrt(ideal_distribution.sum { |grade, ideal_num|
  ...
  class Curve
    ...
    # grades to raw score cutoffs. The scores should be a hash mapping exam IDs
    # (or other opaque identifiers) to raw score numbers. The map should be
    # ordered from highest grade to lowest.
    #
    ...
    #
    # Computes a series of statistics for awarded letter grades. For each letter
    # grade, the following information is returned as a hash:
    #
    ...
    #          score for the previous (higher) letter grade. No value is given for
    #          the highest letter grade.
    def stats
